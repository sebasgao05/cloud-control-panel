...
def clear_auth_cache_between_tests():
    """Clear the auth cache before each test to prevent cross-test contamination."""
    from auth import clear_auth_cache
...
TEST_ADMIN_KEY_HASH = bcrypt.hashpw(
    TEST_ADMIN_KEY.encode("utf-8"), bcrypt.gensalt()
).decode("utf-8")
TEST_ADMIN_KEY_ID = "key-id-admin"

TEST_SUPERADMIN_KEY = "superadmin-key-plaintext"
TEST_SUPERADMIN_KEY_HASH = bcrypt.hashpw(
    TEST_SUPERADMIN_KEY.encode("utf-8"), bcrypt.gensalt()
).decode("utf-8")
